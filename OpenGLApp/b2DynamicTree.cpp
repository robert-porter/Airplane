...
int32 b2DynamicTree::CreateProxy(const b2AABB& aabb, void* userData)
{
	...
	// Fatten the aabb.
	Vector2 r(b2_aabbExtension, b2_aabbExtension);
...
bool b2DynamicTree::MoveProxy(int32 proxyId, const b2AABB& aabb, const Vector2& displacement)
{
	assert(0 <= proxyId && proxyId < m_nodeCapacity);
	...
	Vector2 d = b2_aabbMultiplier * displacement;
...
void b2DynamicTree::InsertLeaf(int32 leaf)
{
	...
	Vector2 center = m_nodes[leaf].aabb.GetCenter();
	...
	if (m_nodes[sibling].IsLeaf() == false)
	{
		do 
		{
			...
			Vector2 delta1 = b2Abs(m_nodes[child1].aabb.GetCenter() - center);
			Vector2 delta2 = b2Abs(m_nodes[child2].aabb.GetCenter() - center);
